...
где $\mathbf{g}=(g_1,g_2)^{\mathrm{T}}$. Определитель — вронскиан столбцов ФСР (равен $\det\Phi$).
...
1. Выписать систему на $C_1',C_2'$ для $\mathbf{g}=(t,1)^{\mathrm{T}}$, если 

$$\Phi=\begin{pmatrix}e^{t}&e^{-t} \\\\ e^{t}&-e^{-t}\end{pmatrix}$$
