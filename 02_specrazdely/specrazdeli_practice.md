...
**Пример.** $A_e=\begin{pmatrix}0&-1\\1&0\end{pmatrix}$ (поворот $90°$), новый базис $e'_1=(1,1), e'_2=(1,-1)$ → $C=\begin{pmatrix}1&1\\1&-1\end{pmatrix}$, $C^{-1}=\frac12\begin{pmatrix}1&1\\1&-1\end{pmatrix}$, $A_{e'}=C^{-1}AC =\begin{pmatrix}0&1\\-1&0\end{pmatrix}$.
...
**Якоби Пример.** $A=\begin{pmatrix}2&1&0\\1&2&1\\0&1&2\end{pmatrix}$, $Δ_1=2$, $Δ_2=3$, $Δ_3=4$ → $Q=2y_1^2+\frac32 y_2^2+\frac43 y_3^2$. Если все $Δ_k≠0$ — быстро.

## 7. Сильвестр — практика — [лекция](https://mathprofi.ru/kvadratichnye_formy.html)

**Пример 1.** $A=\begin{pmatrix}2&-1\\-1&2\end{pmatrix}$: $Δ_1=2>0$, $Δ_2=3>0$ → положительно определена, $Q>0$.

**Пример 2.** $A=\begin{pmatrix}-1&1\\1&-2\end{pmatrix}$: $Δ_1=-1<0$, $Δ_2=1>0$ → отрицательно определена.

**Пример 3.** $A=\begin{pmatrix}1&2\\2&1\end{pmatrix}$: $Δ_1=1>0$, $Δ_2=-3<0$ → неопределённая (седло).

**Пример 4.** $A=\begin{pmatrix}1&1\\1&1\end{pmatrix}$: $Δ_1=1>0$, $Δ_2=0$ → полуопределённая, Сильвестр не решает окончательно → приводим к канону $Q=(x_1+x_2)^2≥0$.
